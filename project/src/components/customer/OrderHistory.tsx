...
// ✅ Fix: Omit 'store' from Order and redefine it
interface OrderWithDetails extends Omit<Order, 'store'> {
  store?: { name: string } | null;
  items?: (OrderItem & {
    product?: { name: string; barcode: string; expiry_date?: string | null };
  })[];
}
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          items:order_items(
            *,
            product:products(name, barcode, expiry_date)
          )
        `)
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusColor[order.status]}`}
                    >
                      {order.status}
                    </span>
                  </div>
                  <p className="text-slate-400 text-xs">
                    {order.store?.name ?? '—'} &bull;{' '}
                    {new Date(order.created_at).toLocaleDateString()}
                  </p>
                </div>
                <p className="text-slate-800 font-bold text-sm flex-shrink-0">
              ...
              {expandedId === order.id && order.items && (
                <div className="border-t border-slate-100 px-5 py-4 bg-slate-50/50 space-y-2">
                  {order.items.map((item) => {
                    const expiry = item.product?.expiry_date ?? null;
